...
import { assignedPreset } from "../lib/worker-ledger.mjs";
import type {
...
export function createPresetLookupAccessors(db: PresetDb, now: () => number) {
  ...
  return {
    ...
    hasCardPreset: (cardId: string) => Boolean(assignedPreset(db, cardId)),
  };
}

export function createPresetSelectionAccessors(
  db: PresetDb,
  lookups: ReturnType<typeof createPresetLookupAccessors>,
) {
  const getPresetForCard = (cardId: string): PresetRow =>
    lookups.getPresetById(assignedPreset(db, cardId)?.preset_id ?? "")
    ?? lookups.getDefaultPreset();
  const getPresetForBand = (band: string, cardId: string): PresetRow => {
    const pinned = lookups.getPresetById(assignedPreset(db, cardId)?.preset_id ?? "");
    if (pinned) return pinned;
    return lookups.getPresetById(bandPresetId(db, band) ?? "") ?? getPresetForCard(cardId);
  };
  return {
    getPresetForCard,
    getPresetForBand,
    getReliablePresetForBand(band: string, cardId: string): PresetRow {
      const resolved = resolveReliablePreset({
        cardPin: assignedPreset(db, cardId)?.preset_id ?? null,
        reliableOverride: lookups.getReliablePresetId(),
        bandPreset: bandPresetId(db, band),
        defaultPreset: null,
      });
      return lookups.getPresetById(resolved.presetId ?? "") ?? getPresetForCard(cardId);
    },

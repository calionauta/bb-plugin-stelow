...
import type { WorkflowDependencyHandlers } from "./workflow-dependency.js";
...
type PlatformDeps = {
  ...
  workflowDependency: WorkflowDependencyHandlers;
};
...
export function createPlatformHandlers(deps: PlatformDeps) {
  return {
    // The BB Workflows dependency ships with the platform surface: the About
    // card is where its status and its two setup actions live.

...
assert.match(server, /buildScopeXray\(map as ScopeMap/, "card detail builds X-ray from the persisted map");
assert.doesNotMatch(server, /join\(sourcePath, "scope-map\.json"\)/, "card detail never falls back to a shared project-root map");
assert.match(server, /parseCurrentShapeVersion\(stateFile\?\.content \?\? null\)/, "card detail uses card state evidence for freshness");

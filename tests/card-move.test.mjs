...
for (const kind of ["research", "explore"]) {
  ...
  for (const phase of ["analysis", "planning", "execution", "review", "completed"]) {
    ...
    assert.match(refused.error, /Inbox, Doing, Done/, `${kind} refusal names the exit (${phase})`);
  }
}
...
// Inbox: a parked build card can move back to it, but a card with a live
// worker cannot — parking it would orphan the worker.
assert.deepEqual(resolveCardMove("build", "inbox"), { ok: true, move: { type: "status", status: "draft" } }, "build enters the Inbox only when unstarted");
...
assert.match(parked.error, /Inbox is for cards that have not started/, "the refusal names what Inbox means");
...
for (const column of ["doing", "done"]) {
  ...
  assert.match(refused.error, /Inbox and workflow phases/, `build refusal names the exit (${column})`);
}

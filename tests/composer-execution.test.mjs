...
assert.equal((server.match(/executionInputSources: \{ providerId: "explicit", model: "explicit", reasoningLevel: "explicit", permissionMode: "explicit" \}/g) ?? []).length, 4, "only restart/reseed/review/draft keep the hardcoded preset-explicit sources; the initial spawn forwards the composer's");

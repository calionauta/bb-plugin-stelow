...
assert.ok(getDecisionPoint(DECISION_POINT_TRIAGE_INTENT).rules.includes("card worker"), "triage rules name who judges (card worker, not the host)");
assert.ok(getDecisionPoint(DECISION_POINT_TRIAGE_INTENT).rules.includes("card pin, reliable override"), "triage rules name the full preset precedence");

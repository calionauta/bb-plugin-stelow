...
function workflowDependencyStub(calls) {
  const status = {
    id: "workflows",
    name: "BB Workflows",
    installed: false,
    ...
    version: null,
    action: "install",
    detail: "Install BB Workflows to unlock durable native execution.",
  };
  const settle = (name) => async () => {
    calls.push(["workflows", name]);
    return { ok: true, error: null, status };
  };
  return {
    workflowDependencyStatus: async () => status,
    installWorkflowDependency: settle("install"),
    enableWorkflowDependency: settle("enable"),
  };
}
...
function harness(overrides = {}) {
  ...
  const deps = {
    ...
    workflowDependency: workflowDependencyStub(calls),
    ...overrides,
  };

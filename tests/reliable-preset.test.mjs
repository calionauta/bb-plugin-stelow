...
// Singleton tables share one constrained DDL and cascade designation rows
// when their preset is deleted. The direct SQLite behavior test executes both
// guarantees; these pins only keep the shared migration wired to all tiers.
assert.match(
  presetMigrations,
  /\["review_preset", "generation_preset", "reliable_preset"\] as const/,
  "all singleton tiers share the singleton migration",
);
assert.match(
  presetMigrations,
  /id INTEGER PRIMARY KEY CHECK \(id = 1\)[\s\S]*FOREIGN KEY \(preset_id\) REFERENCES presets\(id\) ON DELETE CASCADE/,
  "the singleton designation is constrained and cascade-cleared",
);
...
assert.match(
  ...
  "the reliable setter fans out across all live workers",
);
assert.match(
  presetAccessors,
  /refreshRestartPending\([\s\S]*effective\.id/,
  "live workers recompute restart-pending against the new effective preset",
);
...
assert.match(presetHandlers, /getReliablePreset: async \(\) => readSingleton\("reliable_preset"\)/, "the reliable getter RPC exists");
assert.match(
  presetHandlers,
  /assignReliablePreset: async \(\{ presetId \}: SingletonInput\) =>[\s\S]*designate\("reliable_preset", presetId\)/,
  "the reliable setter RPC exists",
);
assert.match(presetHandlers, /INSERT OR REPLACE INTO \$\{table\} \(id, preset_id, assigned_at\)/, "the shared setter upserts one singleton row");
...
const reliableDefAt = presetAccessors.indexOf("const getReliablePresetForBand");
const reliableBodyEnd = presetAccessors.indexOf("\n  };", reliableDefAt);
const reliableBody = presetAccessors.slice(reliableDefAt, reliableBodyEnd);
...
const bandDefAt = presetAccessors.indexOf("const getPresetForBand = (band: string, cardId: string): PresetRow =>");
const bandBodyEnd = presetAccessors.indexOf("\n  };", bandDefAt);
const bandBody = presetAccessors.slice(bandDefAt, bandBodyEnd);
assert.ok(bandDefAt >= 0 && bandBodyEnd > bandDefAt, "the pure band resolver still exists");

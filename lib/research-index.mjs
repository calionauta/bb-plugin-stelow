/**
 * Deterministic parsing for the research index convention.
 *
 * A research worker writes research-index.md in this shape:
 *
 *   ## Opportunities
 *   ### <Strategy label> — <date>
...
export function parseResearchIndex(markdown) {
 ...
 */
export function checkIndexItems(markdown, ids) {
  const wanted = new Set(Array.isArray(ids) ? ids : []);
  if (wanted.size === 0) return { updated: String(markdown ?? ""), checked: [] };
  const { opportunities } = parseResearchIndex(markdown);
  const lines = String(markdown ?? "").split("\n");
  const checked = [];

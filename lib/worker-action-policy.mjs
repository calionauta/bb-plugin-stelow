...
export function workerActionPolicy(card, needsAttention = false) {
  ...
  return {
    ...
    showDelete: archived,
    // Discard is offered everywhere except terminal Done: preview gates on
    // git evidence, so a clean checkout refuses honestly instead of hiding.
    showDiscard: card?.status !== "completed" && card?.status !== "blocked",
  };
}

...
export function acquireGithubImportClaim(db, { key, repo, number, label, token, now }) {
  ...
  // A row is claimable while nothing is linked to it OR while the card it
  // points at is gone: liveImportedKeys reads a dangling card_id as
  // not-imported, so the write side must agree or the issue is offered as
  // importable and refused as in-flight forever.
  const claimed = db.prepare(
    "UPDATE github_imports SET claimed_by = ? WHERE issue_key = ? AND claimed_by IS NULL"
    + " AND (card_id IS NULL OR card_id NOT IN (SELECT id FROM cards))",
  ).run(token, key);

...
// "jev" is the state+questions schema (Typesafe, djev, and compatibles,
// key required); "classifier" is the labels schema (classifier.dev and
// compatibles, keyless). Unknown providers degrade to jev — a misconfigured
// name never reroutes to an unintended shape.
export const DECISION_PROVIDERS = ["jev", "classifier"];
...
export function normalizeDecisionProvider(value, fallback = "jev") {
  if (value === "jev" || value === "classifier") return value;
  return fallback;
}

export function providerRequiresKey(provider) {
  return normalizeDecisionProvider(provider) !== "classifier";
}

export function defaultEndpointFor(provider) {
  return normalizeDecisionProvider(provider) === "classifier" ? CLASSIFIER_DEFAULT_ENDPOINT : DECISION_API_DEFAULT_ENDPOINT;
}
...
export function buildProbeCall(provider) {
  ...
  if (normalizeDecisionProvider(provider) === "classifier") {

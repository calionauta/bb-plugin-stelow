...
const VALUE_FLAGS = new Set(["--scope", "--project", "--name", "--iteration", "--actual-files"]);
const FORWARD_VALUE_FLAGS = ["--name", "--iteration", "--actual-files"];
const USAGE = "Usage: bb stelow scope <start|done> --scope <id> [--project <proj_id>] [--name <workflow>] [--iteration <n>] [--actual-files <a,b>] [--json]";

export function parseScopeArgs(argv) {
  const args = Array.isArray(argv) ? argv : [];
  const op = args[0];
  if (op !== "start" && op !== "done") return { error: USAGE };
  const flag = (name) => {
  ...
  const consumed = new Set(["--scope", "--project", "--name", "--iteration", "--actual-files", "--json", scopeId]);
  let skipNext = false;

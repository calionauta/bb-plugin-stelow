const ROUTES = new Set(["interface-refinement", "shape-contrast", "research-needed", "lean-single-proposal", "stop-and-name-decision", "human-or-shape-stop", "repair-focal-brief", "existing-interface-no-comparison", "repair-brief"]);
...
const ROUTE_RULES = new Map([
  ["interface-refinement", { destination: "interface", staleArtifacts: [], requiresApproval: false, allowed: new Set(["generation-ready:continue", "repair-brief:repair-brief"]) }],
  ["shape-contrast", { destination: "shape", staleArtifacts: ["scope-map", "interface-contrasts", "selection"], requiresApproval: true, allowed: new Set(["stop:human-decision-required", "stop:shape-required"]) }],
  ["research-needed", { destination: "research", staleArtifacts: ["selection", "technical-plan"], requiresApproval: false, allowed: new Set(["stop:research-required"]) }],
  ["lean-single-proposal", { destination: "interface", staleArtifacts: [], requiresApproval: false, allowed: new Set(["generation-ready:continue"]) }],
  ["stop-and-name-decision", { destination: "human", staleArtifacts: ["scope-map", "interface-contrasts", "selection", "technical-plan"], requiresApproval: true, allowed: new Set(["stop:human-decision-required"]) }],
  ["human-or-shape-stop", { destination: "shape", staleArtifacts: ["scope-map", "interface-contrasts", "selection", "technical-plan"], requiresApproval: true, allowed: new Set(["stop:human-decision-required"]) }],
  ["repair-focal-brief", { destination: "interface", staleArtifacts: [], requiresApproval: false, allowed: new Set(["repair-brief:repair-brief"]) }],
  ["existing-interface-no-comparison", { destination: "interface", staleArtifacts: [], requiresApproval: false, allowed: new Set(["no-comparison:continue"]) }],
  ["repair-brief", { destination: "interface", staleArtifacts: [], requiresApproval: false, allowed: new Set(["repair-brief:repair-brief"]) }],
]);
...
export function validateInterfaceContrastReceipt(receipt) {
  ...
  if (ROUTES.has(receipt.route)) {
    try { resolveInterfaceContrastRoute(receipt); } catch (error) { issues.push(error instanceof Error ? error.message : "route/disposition combination is invalid"); }
  }
  ...
  for (const [index, constraint] of (receipt.fixedConstraints ?? []).entries()) {
    if (!constraint || !text(constraint.name) || !text(constraint.value) || !text(constraint.source)) issues.push(`fixedConstraints[${index}] requires name, value, and source`);
  }
  ...
  for (const [index, item] of (receipt.evidence ?? []).entries()) {
    if (!item || !EVIDENCE_SOURCES.has(item.source) || !text(item.reference) || !text(item.claim)) issues.push(`evidence[${index}] requires a supported source, reference, and claim`);
  }
  if (receipt.authority === "agent" && (receipt.evidence ?? []).some((item) => item?.source === "human")) issues.push("agent-authored receipts cannot carry human evidence");
  ...
  if (receipt.briefStatus === "generation-ready") {
    ...
    for (const [index, option] of (receipt.options ?? []).entries()) {
      if (!option || !text(option.id) || !text(option.primaryValue) || option.compatibility !== "valid") issues.push(`options[${index}] must be a valid eligible option`);
    }
  }

 ...
 *
 * `paired` and `effectiveUrl` come from bb itself (`bb connect status --json`
 * and the server's own server-access settings), so the copy that depends on
 * them is never invented.
 */
export function previewHints({ paired, reach, effectiveUrl, hasServerAccessProvider } = {}) {
  const hints = [];
  if (!paired) {
    hints.push({
      tone: "info",
      text: "Pair bb connect to open this from another device. Localhost works here meanwhile.",
      action: "Pair bb connect",
    });
  }
  ...
  if (reach?.provider === "declared") {
    hints.push({ tone: "info", text: "Using the URL declared in .stelow/preview.json.", action: null });
  }
  if (!hasServerAccessProvider && effectiveUrl) {
    hints.push({
      tone: "info",
      text: `This server publishes itself at ${effectiveUrl}. Point the project's preview.json at it if the dev server lives on another machine.`,
      action: null,
    });
  }
  return hints;
...
/** A preview's lifecycle, as the panel renders it. */

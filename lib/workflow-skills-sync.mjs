...
function assertSingleFilePresent(entry, { repoPath, optional, log }) {
  if (entry) return;
  if (optional) {
    log(`stelow file sync: ${repoPath} absent at pinned commit; preserving local copy`);
    return;
  }
  throw new Error(`no ${repoPath} in repo tree`);
}
...
const SINGLE_FILES = [
  ...
  // Optional on pins created before the generated catalog existed. A missing
  // remote file preserves the last good local copy; it is never deleted.
  { repoPath: "skills/stelow-workflow-orchestrator/stage-catalog.json", stateKey: "skills/stelow-workflow-orchestrator/stage-catalog.json", localRel: "data/stelow-stage-catalog.json", optional: true },
  { repoPath: "skills/stelow-workflow-orchestrator/recipe-catalog.json", stateKey: "skills/stelow-workflow-orchestrator/recipe-catalog.json", localRel: "data/stelow-recipe-catalog.json", optional: true },
  { repoPath: "recipes.schema.json", stateKey: "recipes.schema.json", localRel: "data/stelow-recipes.schema.json", optional: true },
];

const EXECUTION_ASSET_PREFIXES = ["recipes/", "schemas/"];

async function syncExecutionAssets({ blobs, source, state, repoRoot, log, result }) {
  for (const blob of blobs.filter(({ path }) => EXECUTION_ASSET_PREFIXES.some((prefix) => path.startsWith(prefix))).sort((a, b) => a.path.localeCompare(b.path))) {
    if (state[blob.path] === blob.sha) continue;
    try {
      const content = await fetchVerifiedBlob(source.commit, blob.path, blob.sha);
      const abs = join(repoRoot, "data", "stelow-assets", blob.path);
      const existed = existsSync(abs);
      mkdirSync(dirname(abs), { recursive: true });
      const tmp = `${abs}.tmp.${process.pid}`;
      writeFileSync(tmp, content);
      renameSync(tmp, abs);
      state[blob.path] = blob.sha;
      (existed ? result.updated : result.created).push(blob.path);
      log(`stelow asset sync: ${blob.path} ${existed ? "updated" : "created"}`);
    } catch (error) {
      result.errors.push(`${blob.path}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export async function syncHelperScript(repoRoot, { log = () => {}, statePath } = {}) {
  ...
  try {
    ...
    const state = loadState(stateFile);
    let dirty = false;
    for (const { repoPath, stateKey, localRel, optional = false } of SINGLE_FILES) {
      const entry = blobs.find((b) => b.path === repoPath);
      assertSingleFilePresent(entry, { repoPath, optional, log });
      if (!entry) continue;
    ...
    await syncExecutionAssets({ blobs, source, state, repoRoot, log, result });

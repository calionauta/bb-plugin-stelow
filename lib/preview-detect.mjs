 ...
 * cannot infer.
 *
 * Host normalization is not re-implemented here: `browseHost` / `localPreviewUrl`
 * in preview-reach own the one definition of loopback and of a local address.
 */

import { browseHost, localPreviewUrl } from "./preview-reach.mjs";
 ...
 * page with no server of its own — a self-contained deliverable that is still
 * worth looking at, served from its own directory on loopback.
 */
export const PREVIEW_TIERS = ["declared", "framework", "static"];

/**
 * Directories never worth descending into when looking for the app: build
 * output, dependencies, and bb/stelow's own scaffolding. A name starting with
 * a dot is skipped too, so `.git`, `.stelow` and friends never become a
 * candidate.
 */
export const PREVIEW_SKIP_DIRS = ["node_modules", "skills", "vendor", "dist", "build", "target", "__pycache__", "venv", ".venv", "tmp"];

/**
 * Depth-1 directories that could hold the app, in a stable order. Agents
 * routinely create the deliverable in a subdirectory, so a workspace that
 * detects nothing at its root is not the end of the question.
 */
export function previewAppDirs(names) {
  return (Array.isArray(names) ? names : [])
    .filter((name) => typeof name === "string" && name.length > 0)
    .filter((name) => !name.startsWith(".") && !PREVIEW_SKIP_DIRS.includes(name))
    .sort();
}

/**
 * Choose which detecting candidate directory is THE app, or `null` when the
 * answer is not obvious.
 *
 * Conventions, in order: the directory named after the card wins, because
 * agents name the folder after the work; a single candidate is unambiguous;
 * anything else is left alone rather than guessed at, and
 * `.stelow/preview.json` remains the way to say which one was meant.
 */
export function pickAppDir(detecting, slug = null) {
  const names = (Array.isArray(detecting) ? detecting : []).filter((name) => typeof name === "string" && name.length > 0);
  if (names.length === 0) return null;
  if (slug) {
    const wanted = String(slug).trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    const match = names.find((name) => name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") === wanted);
...
export function parsePreviewUrl(text) {
  ...
  if (urls.length) {
    ...
    if (port > 0 && port < 65536) {
      ...
      // open at localhost, or the link is dead on arrival. `browseHost` owns
      // that mapping — one definition of "loopback" for the whole feature —
      // and the URL parser is what reads a bracketed IPv6 host correctly.
      let host = match[0].split("//")[1].split(":")[0];
      try { host = new URL(match[0]).hostname; } catch { /* not a parseable URL; the split above stands in */ }
      return { url: `http://${browseHost(host)}:${port}`, port };
    }
  }
  const port = [...text.matchAll(/(?:listening|running|serving) (?:on|at) (?:port |:)?(\d{2,5})\b/gi)].pop();
  if (port) {
    const url = localPreviewUrl(Number(port[1]));
    if (url) return { url, port: Number(port[1]) };
  }
  return null;
}
...
export function previewReady(output, port) {
  ...
  if (port && /(ready in|compiled successfully|listening|started server|running on)/i.test(output)) {
    const url = localPreviewUrl(port);
...
function detectStatic(snapshot) {
  const page = ["index.html", "index.htm"].find((rel) => snapshot.exists(rel));
  if (!page) return null;
  // `--bind` is spelled out rather than inherited: a self-contained page is
  // still served from a real directory, and it must be loopback like every
  // other preview.
  return {
    framework: "static",

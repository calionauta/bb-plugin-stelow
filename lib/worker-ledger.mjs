...
/**
 * Flag a thread that provably predates the current preset override (born
 * before it was assigned). Returns true when the flag was raised.
 * Never clears — only spawn paths clear, so a flagged card keeps offering
 * Restart until it actually happens.
 */
export function assignedPreset(db, cardId) {
  return db.prepare("SELECT preset_id, assigned_at FROM card_presets WHERE card_id = ?").get(cardId);
}
...
export function healPresetStaleness(db, cardId, threadBorn, flagSet) {
  ...
  const override = assignedPreset(db, cardId);

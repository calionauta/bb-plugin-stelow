...
/** Find a contract by one of its id keys, tolerating "not migrated yet". */
...
export const EXPLORE_CONTRACTS = [
  {
    ...
    minWords: 800,
    checks: [
      { kind: "named-headings", level: null, names: ["unanswered questions", "alternatives", "problem", "solution", "dangers", "out of scope", "Scope Table"], match: "contains" },
  ...
  },
  {
    ...
    minWords: 800,
    checks: [
      { kind: "named-headings", level: null, names: ["Work Pattern", "Philosophy", "Trade-Off", "State Coverage"], match: "contains" },
      { kind: "table-rows", min: 2 },
  ...
  },
  {
    ...
    minWords: 800,
    checks: [
      { kind: "named-headings", level: null, names: ["Summary", "Gap Registry", "Lessons Learned", "Decision"], match: "contains" },
      { kind: "table-rows", min: 4 },
      { kind: "gap-registry" },
    ],
  },
...
export function contractForBuildArtifact(filePath, content) {
  ...
  if (base === "audit.md" || base === "state.md" || base.endsWith("-approved.md")) return null;
  const lower = base.toLowerCase();
  const fullLower = String(filePath ?? "").toLowerCase();
  if (fullLower.includes("codebase-critique") || lower.includes("codebase")) return contractForExplore("codebase-critique");
  if (fullLower.includes("ux-critique") || lower.includes("live-audit") || lower.includes("screenshot-audit")) return contractForExplore("ux-critique");
  for (const rule of BUILD_PATH_RULES) {
    if (lower.includes(rule.match)) return contractForExplore(rule.contract);
  }
  const body = typeof content === "string" ? content : "";
  for (const rule of BUILD_TITLE_RULES) {
    if (body.includes(rule.match)) return contractForExplore(rule.contract);
  }
  return null;
}

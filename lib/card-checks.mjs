...
// Missing-tracking signal (pure, advisory): no synced scopes at all while
// the card runs or sits past planning is the incident shape — the worker
// is executing (or already executed) with nothing to mark. Unlike
// isExecutionUntracked, which needs synced scopes to compare against,
// emptiness here IS the alarm. Planning stages and idle cards stay silent.
const POST_PLANNING_STAGES = new Set(["execution", "verification", "audit"]);
export function isScopeTrackingMissing({ activity, stage, scopes }) {
  const list = Array.isArray(scopes) ? scopes.filter((scope) => scope && typeof scope === "object") : [];
  if (list.length > 0) return false;
  if (activity === "running") return true;
  return typeof stage === "string" && POST_PLANNING_STAGES.has(stage);
}

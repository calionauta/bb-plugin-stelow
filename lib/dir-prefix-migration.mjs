...
/**
 * Shared-hash reunion: two cards seeded from the same hash (clone/reseed)
 * migrate as one directory — the first renames it, the second finds its
 * pw- source gone. Its sw- counterpart in tracking IS its directory, so
 * the card adopts the hash with a metadata-only update (no second move).
 * The sw- side can only come from that same pw- seed: hashes were unique
 * per seed, so the match is still an identity match.
 */
export function migratedCounterpartForHash(workflows, dirHash) {
  const target = swDirHash(dirHash);
  if (!text(dirHash).startsWith("pw-")) return null;
  const list = Array.isArray(workflows) ? workflows : [];
  return list.find((raw) => {
    const entry = raw !== null && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
    return text(entry.dirHash) === target;
  }) ?? null;
}
 ...
 * their unique stored hash alone — the hash was unique per seed, so a dirHash

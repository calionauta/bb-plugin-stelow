 ...
 * - research: lightweight Inbox / Doing / Done investigations, one strategy
 *             round at a time, producing research-index.md + round files.
 * - explore:  lightweight Inbox / Doing / Done single-stage runs, producing
...
import { BUILD_BOARD_COLUMNS } from "./workflow-vocabulary.mjs";

export const CARD_KINDS = ["build", "research", "explore"];

/** Tracks with the lightweight Inbox / Doing / Done lifecycle (no stages). */
export const LIGHTWEIGHT_KINDS = ["research", "explore"];

/**
 * Board columns for the lightweight lifecycle, shared by Research + Explore.
 *
 * The first column is the Inbox — the one word for "captured, nothing running
 * yet" on every track (Build's board reads the same name). It replaced a
 * "To-Do" that described work the user had already decided to do.
 */

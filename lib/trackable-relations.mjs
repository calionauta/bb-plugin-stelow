 ...
 * ids pass through untouched); flat entries register as-is with kind
 * defaulting to "trackable". Later ids never overwrite earlier ones.
 */
export function buildRegistry(entries) {
  const registry = new Map();
  const put = (entry) => {
    const id = entry && typeof entry === "object" ? cleanId(entry.id) : null;
  ...
  for (const entry of Array.isArray(entries) ? entries : []) {
    ...
    for (const task of tasks) {
      const taskId = task && typeof task === "object" ? cleanId(task.id) : null;
      if (!taskId) continue;
      taskIds.push(taskId);
      put({ ...task, id: taskId, kind: task.kind ?? "task", parent: cleanId(entry.id) ?? undefined });
    }
    put({ ...entry, kind: entry.kind ?? (tasks.length > 0 || children.length > 0 ? "scope" : "trackable"), children: [...children, ...taskIds] });
  }
  return registry;
}

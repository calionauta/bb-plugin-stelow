 ...
 * to stop opening it.
 *
 * Deliberately separate from the action badge below — finished work is not
 * blocked work, and one flag with two meanings is how a precise signal turns
 * into noise. The completion's own read state is the lifecycle: completion
 * writes it unread, and opening the Done card clears it.
 */

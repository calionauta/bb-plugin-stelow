...
export function ensureInboxSeverityColumns(db) {
  const columns = db.prepare("PRAGMA table_info(inbox_events)").all();
  if (!columns.some((column) => column.name === "severity")) {
    db.exec("ALTER TABLE inbox_events ADD COLUMN severity INTEGER NOT NULL DEFAULT 1");
  }
  if (!columns.some((column) => column.name === "severity_reasons")) {
    db.exec("ALTER TABLE inbox_events ADD COLUMN severity_reasons TEXT NOT NULL DEFAULT '[]'");
  }
}

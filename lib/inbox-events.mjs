...
export function syncQuestionInboxEvents(db, { cardId, interactionIds, occurredAt, createId, summary }) {
  ...
  // A card cannot simultaneously need a generic Resume and a specific
  // answer. The latter tells the person exactly what to do, so resolve every
  // open pause as superseded rather than turning one card into several badge
  // counts for the same attention need.
  const pausedSuperseded = ids.length > 0
    ? resolveActionInboxEvents(db, cardId, occurredAt, ["paused"], "superseded")
    : 0;

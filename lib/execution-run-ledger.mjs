...
export function transitionExecutionRun(db, id, next, patch = {}) {
  ...
  if (current.normalizedStatus === next) {
    if (patch.runId || patch.nativeStatus || patch.errorCode || patch.previewDirective || patch.completionEventId || patch.needsInputSentAt || patch.resumeRequestedAt || patch.boundaryId || patch.boundaryQuestion) {
      db.prepare(`UPDATE execution_runs SET run_id = COALESCE(?, run_id), native_status = COALESCE(?, native_status), error_code = COALESCE(?, error_code), preview_directive = COALESCE(?, preview_directive), completion_event_id = COALESCE(?, completion_event_id), needs_input_sent_at = COALESCE(?, needs_input_sent_at), resume_requested_at = COALESCE(?, resume_requested_at), boundary_id = COALESCE(?, boundary_id), boundary_question = COALESCE(?, boundary_question) WHERE id = ?`)
        .run(patch.runId ?? null, patch.nativeStatus ?? null, patch.errorCode ?? null, patch.previewDirective ?? null, patch.completionEventId ?? null, patch.needsInputSentAt ?? null, patch.resumeRequestedAt ?? null, patch.boundaryId ?? null, patch.boundaryQuestion ?? null, id);
  ...
  db.prepare(`UPDATE execution_runs SET run_id = COALESCE(?, run_id), native_status = COALESCE(?, native_status), normalized_status = ?, completed_at = ?, error_code = ?, preview_directive = ?, completion_event_id = COALESCE(?, completion_event_id), needs_input_sent_at = COALESCE(?, needs_input_sent_at), resume_requested_at = COALESCE(?, resume_requested_at), boundary_id = COALESCE(?, boundary_id), boundary_question = COALESCE(?, boundary_question) WHERE id = ?`)
    .run(patch.runId ?? null, patch.nativeStatus ?? null, next, completedAt, patch.errorCode ?? null, patch.previewDirective ?? null, patch.completionEventId ?? null, patch.needsInputSentAt ?? null, patch.resumeRequestedAt ?? null, patch.boundaryId ?? null, patch.boundaryQuestion ?? null, id);
  return getExecutionRun(db, id);
}

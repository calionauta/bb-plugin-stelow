...
// A challenge's disposition is the same vocabulary the map's own routes use, so
// one set decides it everywhere: a disposition the router does not know is a
// disposition no downstream stage could act on.
const CHALLENGE_DISPOSITIONS = new Set([
  "continue",
...
export function validateScopeMapChallenge(challenge) {
  ...
  if (!CHALLENGE_DISPOSITIONS.has(challenge.disposition)) issues.push("disposition is not supported");

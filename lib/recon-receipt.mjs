...
/**
 * Whether the recon receipt deserves a line on an active card (pure).
 * Missing, unreadable, or recorded-but-degraded receipts explain thinner
 * analysis, so they surface; an all-available receipt stays silent.
 */
export function isReconActionable(recon) {
  if (!recon || typeof recon !== "object") return false;
  if (recon.state === "missing" || recon.state === "invalid") return true;
  return recon.state === "recorded" && /unavailable/i.test(typeof recon.detail === "string" ? recon.detail : "");
}

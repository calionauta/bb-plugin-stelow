...
export const RECON_RECEIPT_CONTRACT = "stelow-recon-v1";

export function reconReceiptStatus(content) {
  if (typeof content !== "string" || !content.trim()) return { state: "missing", detail: `No ${RECON_RECEIPT_FILE} was found.` };
  try {
    const receipt = JSON.parse(content);
    if (!receipt || receipt.contract !== RECON_RECEIPT_CONTRACT || receipt.workspace?.git !== true) {

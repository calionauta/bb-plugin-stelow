...
export const TECHNIQUE_CATALOG = [
  ...
  },
  {
    ...
    skill: "stelow-workflow-interface-contrast",
    emoji: "⚖️",
    // The blurb carries the one thing a reader cannot infer: the pick is
    // recorded HERE, on this card, and a decision that becomes a build is owned
    // by that build's selection stage. Explore can host the question; it cannot
    // own the commitment.
    blurb: "Compare bounded interface options against one preserved first reaction, then record a pick here.",
    keywords: ["ui", "interface", "contrast", "compare", "decision", "reaction", "tradeoffs"],
    optionalEvidence: ["interfaces/contrast.json"],
  },
  {
    id: "plan-critique",
  ...
  },
  {
    id: "scope-mapping",
    label: "Scope Map",
    skill: "stelow-product-scope-mapping",
    emoji: "🗺️",
    blurb: "Break a proposal into coherent delivery scopes with explicit boundaries, dependencies, and open decisions.",
    keywords: ["scope", "map", "slices", "boundaries", "planning", "dependencies"],
    primaryArtifact: "explore-scope-map.md",
    optionalEvidence: ["scope-map.json"],
  },
  {

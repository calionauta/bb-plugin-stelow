 ...
 * Inbox: parking a card that is already running would orphan its worker, so
...
export function resolveCardMove(kind, target, { hasWorker = false } = {}) {
  // Research and Explore cards share the lightweight Inbox / Doing / Done /
  ...
  if (isLightweightKind(kind)) {
    ...
    if (!next) return { ok: false, error: "Research and Explore cards move between Inbox, Doing, Done, and Archived — build phases do not apply." };
  ...
  if (target === BUILD_BOARD_INBOX) {
    if (hasWorker) {
      return { ok: false, error: "This card already has a worker — Inbox is for cards that have not started. Move it to a workflow phase, or archive it." };
    }
    return { ok: true, move: { type: "status", status: "draft" } };
  }
  if (LIGHTWEIGHT_COLUMNS.includes(target) && !BUILD_BOARD_TERMINALS.includes(target)) {
    return { ok: false, error: "Build cards move between Inbox and workflow phases — Doing and Done are lightweight columns." };
  }
  if (target === "completed") {

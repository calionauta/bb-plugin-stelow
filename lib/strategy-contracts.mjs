...
/**
 * A strategy whose primary file is a guided section list, plus the required
 * terms when it has any. A strategy with no required terms gets the section
 * list alone: a `contains` check with an empty needle list can never fail, and a
 * check that can never fail is a contract that reads stricter than it is.
 */
function guided(id, ref, minWords, headingCount, terms) {
  return {
    ...
    checks: [
      ...
      ...(terms.length > 0 ? [{ kind: "contains", needles: terms }] : []),
    ],
  };
}

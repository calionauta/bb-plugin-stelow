 ...
 * stelow-product-* playbook (vendored by the upstream skills sync, same as
 * the build worker prompt references). Single source of truth for the
...
/**
 * Merge upstream neutral contracts (`product-strategies.json` from the
 * stelow repo: id → {skill, contract, substeps}) onto a local presentation
 * list. Returns a NEW array; unknown upstream ids are ignored (the local
 * list curates what the picker offers); local presentation fields
 * (label, blurb, emoji, keywords) always win. Pure — tested, no I/O.
 */
export function mergeStrategyContracts(localList, registry) {
  const byId = new Map();
  ...
  return (localList ?? []).map((local) => {
    const upstream = byId.get(local.id);
    if (!upstream) return { ...local };
    const merged = { ...local };
    if (typeof upstream.skill === "string" && upstream.skill) merged.skill = upstream.skill;
    if (["single", "variant", "composite"].includes(upstream.contract)) merged.contract = upstream.contract;
    if (merged.contract === "composite" && Array.isArray(upstream.substeps) && upstream.substeps.length > 0) {
      merged.substeps = [...upstream.substeps];
    } else if (merged.contract !== "composite") {
      delete merged.substeps;
    }
    return merged;
  });
}
 ...
 * Other-project decisions also carry the mapped `owner` (project id or
 * null) so dry-runs can name where the issue belongs instead of only
 * where it doesn't.
 */
export function decideAutomationIssue(issue, { watched, trustedAuthors, projectId, projectForRepo, fired, imported }) {
  ...
  if (ownerOf(issue, projectForRepo) !== projectId) {
    return { ok: false, key, reason: "other-project", owner: ownerOf(issue, projectForRepo) };
  }
...
export function previewAutomationMatches(issues, options) {
  ...
  for (const issue of issues ?? []) {
    ...
    if (decision.ok) matches.push({ repo: issue.repo, number: issue.number, key: decision.key });
    else skipped.push({ repo: issue.repo, number: issue.number, key: decision.key, reason: decision.reason, owner: decision.owner ?? null });
  }

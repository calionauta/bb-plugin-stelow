...
/**
 * How long a spawn may stay `starting` before the host calls it. A dev
 * server that never announces an address (wrong command, silent crash,
 * output the readiness scan does not recognize) used to sit in "Starting…"
 * forever with no way to debug it. Past the timeout the session fails with
 * the last log lines attached, so the log viewer becomes the diagnosis.
 */
 ...
 */
export function createPreviewRuntime({ readFile, listDirs, joinPath, spawnProcess, runConnect, now = () => Date.now(), baseEnv = {}, startTimeoutMs = PREVIEW_START_TIMEOUT_MS }) {
  ...
  function clearStartTimer(session) {
    if (session.timer) {
  ...
  async function start(target) {
    ...
    // loudly with its log attached, never sit in "Starting…" forever. The
    // timer is unref'd so tests and idle hosts are never held open by it.
    const startTimer = setTimeout(() => {
      session.timer = null;
      if (session.state !== "starting") return;
      session.state = "failed";
      session.error = `The dev server did not announce an address within ${Math.round(startTimeoutMs / 1000)}s. The last output is in the log — inspect it, then Stop and Start again.`;
    }, startTimeoutMs);
    startTimer.unref?.();
    session.timer = startTimer;
  ...
  /**
   * Retry the Connect share for a live preview. Exposing already runs on
   * every start, but a transient failure leaves a paired host with a
   * localhost-only address and a dead "Share this port" button — this is
   * what that button calls.
   */
  async function share(target) {
    const session = sessionsUnder(target.hostId, target.checkout).find(isLive);
    if (!session) return { ok: false, error: "No live preview to share." };
    const parsed = await runConnect(["expose", String(session.port)]).catch(() => null);
    session.share = parsed ? parseShareExpose(parsed) : null;
  ...
  return { start, stop, view, dispose, share };
}

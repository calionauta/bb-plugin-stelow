...
// random value. A reseed intentionally receives a new generation. The `sw-`
// prefix reads as "a stelow workflow" and unifies both generators: the
// owner-derived hash here and the random hash in upstream `scripts/stelow`
// (seed path). Anything reading a state dir must treat the prefix as opaque
// and resolve through the stored `dirHash`, never by pattern.
export function workflowDirHash(workflowId, fresh = false, timestamp = Date.now()) {
  ...
  return fresh ? `sw-${owner}-${timestamp.toString(36)}` : `sw-${owner}`;
}

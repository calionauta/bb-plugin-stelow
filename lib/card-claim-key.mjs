...
/**
 * Effective-checkout resolution shared by every acquire/check/release call.
 * Managed-worktree scopes resolve to their worktree path; project-default
 * scopes fall back to the project source. Keying by source would
 * false-conflict isolated worktrees (over-lock).
 */
export function resolveClaimCheckout({ checkoutPath, worktreePath, sourcePath } = {}) {
  const pick = (value) => (typeof value === "string" && value.trim() ? value.trim() : null);
  return pick(checkoutPath) ?? pick(worktreePath) ?? pick(sourcePath) ?? null;
}

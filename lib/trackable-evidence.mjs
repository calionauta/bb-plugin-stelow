...
import { matchScopeClaims } from "./card-claims.mjs";
...
export function evidenceConditions({ entry, registry = new Map(), claimed = null, claimLapsed = false } = {}) {
  ...
  const definition = contractForTrackable(entry.kind);
  const expectsContract = Boolean(definition?.contractFile);
  // Records only bind kinds whose contract declares an inline record
  // (scopes). A done task, stage, or question without a Record is normal
  // — their evidence lives elsewhere — so silence beats noise.
  const expectsRecord = Boolean(definition?.recordInline);
 ...
 */
export async function enrichEntriesForDetail({ entries, defaultKind = null, stateRelDir = null, ownerId = null, liveClaims = [], isLapsed = null, readContract = null, nowMs = Date.now() } = {}) {
  const list = Array.isArray(entries) ? entries : [];
  const options = typeof defaultKind === "string" && defaultKind ? { defaultKind } : {};
  const bare = (entry) => ({
    ...entry,
    tasks: (Array.isArray(entry?.tasks) ? entry.tasks : []).map((task) => ({
      ...task,
      conditions: evidenceConditions({ entry: { ...task, kind: "task" }, registry: buildRegistry(list, options) }),
    })),
    conditions: [],
    claimed: null,
  });
  if (!stateRelDir) return list.map(bare);
  const registry = buildRegistry(list, options);
  ...
  return await Promise.all(list.map(async (entry) => {
    ...
    const kind = typeof entry.kind === "string" && entry.kind
      ? entry.kind
      : (typeof defaultKind === "string" && defaultKind ? defaultKind : undefined);
    const rel = contractRelPath(stateRelDir, kind, entry.id);
    ...
    const mine = matchScopeClaims(live, { ownerId: ownerId ?? undefined, scopeId: entry.id, files: targets, nowMs });

...
/**
 * Live rows held by one card for one scope: exact scope tag or target-file
 * overlap, unexpired. Single owner of the matching rule used by both the
 * claimed projection and the lapsed probe below.
 */
export function matchScopeClaims(rows, { ownerId, scopeId, files = [], nowMs = Date.now() } = {}) {
  const targets = uniqueFiles(files);
...
/**
 * Scope-scoped claim tag: batchId namespaces sibling scopes of one card so
 * the holder is always (card, scope), never the bare card.
 */
...
function resolveScopeWorkspace(explicit) {
  if (typeof explicit === "string" && explicit.trim()) return explicit.trim();
  return null;
}

/**
 * Per-scope claim acquire: all-or-nothing across every file. A live foreign
 * holder — including a sibling scope of the same card, which is a distinct
 * holder — refuses the whole acquire with CONFLICT naming the holder. The
 * waiter parks with a BB-LOCK-BLOCKED payload and a
 * lock-blocked:<card>:<file> dedupe key; the host nudges agent-only on
 * release. No retry loop lives here.
 */
export function acquireScopeClaims(db, {
  cardId,
  batchId = null,
  scopeId,
  files,
  ...
  const workspace = resolveScopeWorkspace(effectiveCheckout)
    ?? resolveScopeWorkspace(workspacePath)
    ?? resolveScopeWorkspace(checkoutPath)
    ?? resolveScopeWorkspace(worktreePath)
    ?? resolveScopeWorkspace(sourcePath);
  const tag = scopeClaimTag(batchId, scopeId);
  const targets = uniqueFiles(files);
  if (!cardId || !tag || targets.length === 0 || !workspace) {
    return { ok: false, code: "CLAIM_REQUIRED", acquired: [], conflicts: [], park: [] };
  }
  const select = db.prepare(
    "SELECT card_id, scope, expires_at, fencing FROM card_claims WHERE workspace_path = ? AND file_path = ?",
  );
  const conflicts = [];
  for (const file of targets) {
    const existing = select.get(workspace, file);
    if (!existing || existing.expires_at <= nowMs) continue;
    const sameHolder = existing.card_id === cardId && existing.scope === tag;
    if (sameHolder) continue;
    conflicts.push({
      file,
      heldBy: existing.card_id,
      heldScope: existing.scope,
      holder: `${existing.card_id}::${existing.scope ?? "unscoped"}`,
      expiresAt: existing.expires_at,
    });
  }
  if (conflicts.length > 0) {
    addClaimWaiters(db, { cardId, workspacePath: workspace, files: conflicts.map((entry) => entry.file), scope: tag, nowMs });
    const park = conflicts.map((entry) => ({
      file: entry.file,
      stderr: `BB-LOCK-BLOCKED file=${entry.file} heldBy=${entry.holder} expiresAt=${entry.expiresAt}; park this scope, do not retry in a loop`,
      dedupeKey: `lock-blocked:${cardId}:${entry.file}`,
      visibility: "agent-only",
    }));
    return { ok: false, code: "CONFLICT", acquired: [], conflicts, park };
  }
  const upsert = db.prepare(
    "INSERT INTO card_claims (workspace_path, file_path, card_id, scope, acquired_at, expires_at, fencing) VALUES (?, ?, ?, ?, ?, ?, ?) " +
    "ON CONFLICT(workspace_path, file_path) DO UPDATE SET " +
    "card_id = excluded.card_id, scope = excluded.scope, " +
  ...
  const transact = db.transaction(() => {
    for (const file of targets) {
      const fencing = nextFencing(db, workspace);
      upsert.run(workspace, file, cardId, tag, nowMs, nowMs + ttlMs, fencing);
      acquired.push({ file, fencing, holder: `${cardId}::${tag}` });
    }
  });
  transact();
  return { ok: true, code: "ACQUIRED", acquired, conflicts: [], park: [] };
}

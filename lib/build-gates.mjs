 ...
 *
 * Advance order: untracked-execution → dependency cycle → scope sufficiency
 * (broad refactors need an approved Scope Map) → unstartable ordering note
...
function scopeSufficiencyRefusal({ intent, scopeCount = 0, hasScopeMap = false } = {}) {
  if (intent !== "refactor" || scopeCount <= 1 || hasScopeMap) return null;
  return "Refused: this refactor has multiple delivery scopes but no approved Scope Map — "
    + "return to the Scope stage, record the map and its approval receipt, then advance again.";
}
 ...
 */
export function advanceExecutionGates({ kind, stage, specContent, syncedCount, cycles = [], hasUnstartablePending = false, intent, hasScopeMap = false } = {}) {
  ...
  const scopeRefusal = scopeSufficiencyRefusal({ intent, scopeCount: syncedCount ?? 0, hasScopeMap });
  if (scopeRefusal) return { refusal: scopeRefusal, note: null };

 ...
 * to the fallback. The full SDK union is honored — including
 * provider-provisioned environments (custom machines), which must never
 * silently become the project checkout.
 */
export function selectCardEnvironment(requested, fallback) {
  ...
  if (requested.type === "provider" && typeof requested.environmentProviderId === "string") return requested;
...
function describeRequested(requested) {
  ...
  if (type === "provider") {
    return typeof requested.environmentProviderId === "string"
      ? `provider environment (${requested.environmentProviderId})`
      : "provider environment (missing provider id)";
  }
...
function describeSelected(selected) {
  ...
  if (selected.type === "provider") {
    return typeof selected.environmentProviderId === "string"
      ? `provider environment (${selected.environmentProviderId})`

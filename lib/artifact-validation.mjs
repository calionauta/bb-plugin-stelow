 ...
 * - { kind: "headings", level, min, max?, startsWith?, startsWithAny?, contains? }
...
function checkHeadings(text, check) {
  const all = headingLines(text).filter((heading) => check.level == null || heading.level === check.level)
    ...
    .filter((heading) => !check.startsWithAny || check.startsWithAny.some((stem) => heading.text.toLowerCase().startsWith(stem.toLowerCase())))

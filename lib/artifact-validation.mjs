...
function checkHeadings(text, check) {
  const all = headingLines(text).filter((heading) => heading.level === check.level)
  ...
  if (check.min != null && all.length < check.min) {
    return { code: "too-few-headings", expected: `at least ${check.min} level-${check.level} headings`, found: `${all.length}`, detail: `expected at least ${check.min} level-${check.level} headings, found ${all.length}` };
  }
  if (check.max != null && all.length > check.max) {
    return { code: "too-many-headings", expected: `at most ${check.max} level-${check.level} headings`, found: `${all.length}`, detail: `expected at most ${check.max} level-${check.level} headings, found ${all.length}` };
  }
  return null;
}

function checkNamedHeadings(text, check) {  const all = headingLines(text).filter((heading) => check.level == null || heading.level === check.level);
